...
// Retrieve moments and sort them by publication date
...
// Combine moments and essays for the main post listing
async function getRawSortedPosts() {
	const allMoments = await getRawSortedMoments();
	const allEssays = await getRawSortedEssays();
	
	const allPosts = [...allMoments, ...allEssays];
...
export type PostForList = {
	slug: string;
	data: CollectionEntry<"moments" | "essays">["data"];
};
...
export async function getSortedMomentsList(): Promise<PostForList[]> {
	...
	// delete post.body
	const sortedPostsList = sortedFullMoments.map((post) => ({
		slug: post.slug,
		data: post.data,
	}));
...
export async function getTagList(): Promise<Tag[]> {
	const allMoments = await getCollection<"moments">("moments", ({ data }) => {
		return import.meta.env.PROD ? data.draft !== true : true;
	});
	...
	const allBlogPosts = [...allMoments, ...allEssays];
